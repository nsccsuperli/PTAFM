...
if __name__ == "__main__":
    ...
    for repetition in range(3):
        ...
        for case_idx, simulate_case in enumerate(simulate_cases):
            ...
            candidate_partitions, all_cost_records, min_cost_records = scheduler.GCMA(nodes=list(range(scheduler.num_devices)), 
                                                                                      population_size=100, 
                                                                                      trails=4900, 
                                                                                      mode="baseline")
            ...
            data_parallel_cost = scheduler.compute_data_parallel_cost(candidate_partition=candidate_partition)
            pipeline_parallel_cost, pipeline_parallel_path, pipeline_parallel_match = scheduler.compute_pipeline_parallel_cost(candidate_partition)
            min_total_cost = data_parallel_cost + 2 * pipeline_parallel_cost

            end = time.perf_counter()
            print("run time(" + str(len(all_cost_records)) + " candidates): " + str(end - start) + " seconds")
            ...
            candidate_pipeline = scheduler.get_pipelines(candidate_partition, pipeline_parallel_path, pipeline_parallel_match)
